...
void  Instruction()
{
    ...
    sprintf(ins,"Now,here King means computer and the initial task is to collect above 10 treasure box.");
...
void Music()
{
    ...
    p = 1+(rand()%1000);
    if(p%11==0)
        PlaySound(TEXT("Synth Pop Loop.wav"),NULL, SND_FILENAME|SND_LOOP|SND_ASYNC);
    else if(p%7==0)
        PlaySound(TEXT("Yo Camel!.wav"),NULL, SND_FILENAME|SND_LOOP|SND_ASYNC);
    else if(p%5==0)
        PlaySound(TEXT("Slowfunky 88.wav"),NULL, SND_FILENAME|SND_LOOP|SND_ASYNC);
    else if(p%3==0)
        PlaySound(TEXT("Ekon Beat loop.wav"),NULL, SND_FILENAME|SND_LOOP|SND_ASYNC);
    else
        PlaySound(TEXT("Pim Poy.wav"),NULL, SND_FILENAME|SND_LOOP|SND_ASYNC);
